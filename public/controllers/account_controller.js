...
$scope.account_balance = function() {
         $http.get('http://BalanceAPIIP:3000/balance/?myAccountId='+$cookies.get('email')).success(function(response) {             
		    ...
		    $scope.form.payeeAddress = payee.payeeAddress;
                ...
                $scope.submitPayee = function() {
                    ...
                    if ($scope.form.id == -1) {
                        ...
                        method = "POST";
                        url = 'http://PayeeAPIIP:3000/payee';
                    } else {
                        //If Id is present, it's edit operation - PUT operation
                        method = "PUT";
                        url = 'http://PayeeAPIIP:3000/payee/' + $scope.form.id;
                    }
...
//THE FIELD PAYEE._ID IS REFERENCED FROM THE PAYEE TABLE ITSELF
                ...
                $scope.removePayee = function(payee) {
                    $http({
                        method : 'DELETE',
                        url : 'http://PayeeAPIIP:3000/payee/' + payee._id
                    }).success(function(response) {
location.reload()
});
}
...
$scope.init_payee = function() {
         $http.get('http://PayeeAPIIP:3000/payee/?payeeAddress='+$cookies.get('email')).success(function(response) {             
...
$http({
          method  : 'POST',
          url     : 'http://TransferAPIIP:3000/transfer',
          data    : $scope.payform 
         })
...
$http({
          method  : 'PUT',
          url     : 'http://BalanceAPIIP:3000/balance/'+$scope.my_balanceid,
...
$scope.account_transactions = function() {
         $http.get('http://TransferAPIIP:3000/transfer/?myAccountId='+$cookies.get('email')).success(function(response) {             
...
$http.get('http://UserAPIIP:3000/users/?userId='+username).success(function(response) {
